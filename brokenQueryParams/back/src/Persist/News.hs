...
import Database.Esqueleto.Experimental (Entity (..), From, SqlExpr, SqlQuery, Value (..), asc, distinct, distinctOn, don, in_, innerJoin, just, like, limit, notIn, on, orderBy, selectOne, set, unionAll_, union_, updateCount, valList, withRecursive, (=.), (==.), (||.), type (:&) ((:&)))
...
selectCategories _categoryFilters_include = do
  pure $ table @Categories
  -- distinct $
  --   withRecursive
  --     ( do
  ...
  --                 ( case list_ of
  --                     [] -> val True
  --                     x -> category.id `op` valList (mkSqlKey <$> x)
  --                 )
  --           in check _categoryFilters_include in_ 
